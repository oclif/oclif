...
* **Super Speed** - The overhead for running an oclif CLI command is almost nothing. It includes very few dependencies. Also, only the command to be executed will be required with node. So large CLIs with many commands will load just as fast as a small one with a single command.

...
_See code: [src/commands/pack/deb.ts](https://github.com/oclif/oclif/blob/4.23.32/src/commands/pack/deb.ts)_
...
_See code: [src/commands/pack/macos.ts](https://github.com/oclif/oclif/blob/4.23.32/src/commands/pack/macos.ts)_
...
_See code: [src/commands/pack/tarballs.ts](https://github.com/oclif/oclif/blob/4.23.32/src/commands/pack/tarballs.ts)_
...
FLAGS
  -r, --root=<value>                 (required) [default: .] Path to oclif CLI root.
  -t, --tarball=<value>              Optionally specify a path to a tarball already generated by NPM.
      --defender-exclusion=<option>  [default: unchecked] Set to "checked" or "unchecked" to set the default value for
                                     the checkbox.  Set to "hidden" to hide the option (will let defender do its thing).
...
FLAG DESCRIPTIONS
  --defender-exclusion=checked|unchecked|hidden
    ...
    Defaults to "unchecked" so end users opt in rather than opt out of excluding the install directory from Windows
    Defender scanning. Note: "hidden" always disables the exclusion, since there is no way to set a hidden checkbox with
    "true" as a default...the user can always allow full security
```

_See code: [src/commands/pack/win.ts](https://github.com/oclif/oclif/blob/4.23.32/src/commands/pack/win.ts)_

...
`
  ...
  static flags = {
    ...
    readmePath: Flags.string({default: 'README.md', description: 'Path to the README file.', required: true}),
  ...
  replaceTag(readme: string, tag: string, body: string): string {
    if (readme.includes(`<!-- ${tag} -->`)) {
      ...
      this.log(`replacing <!-- ${tag} --> in ${this.flags.readmePath}`)
    }
  ...
  async run(): Promise<void> {
    ...
    const readmePath = path.resolve(cwd, flags.readmePath)

...
describe('readme', () => {
  ...
  describe('with command that is pure Javascript', () => {
    const rootPath = path.join(__dirname, '../fixtures/cli-command-javascript')
    ...
    .command(['readme'])
    .it('has a space topic separator', () => {
      const newReadme = fs.readFileSync(readmePath, 'utf8')
      expect(newReadme).to.contain('src/commands/hello.js')
      expect(newReadme).to.contain('src/commands/do/something.js')
    })
  })
